 ...
 *       Filename:  wrapsock.c
 *
 *    Description:  Socket wrapper function.
 *    These could all go into separate files, so only the ones
 *    needed cause the corresponding function to be added to the executable.
 *    If sockets are a library (SVR4) this might make a difference(?), but
 *    if sockets are in the kernel (BSD) it doesn't matter.
 *
 ...
 *    warning: passing arg 2 of `bind` discards `const` from pointer target
 *    type, warning: passing arg 2 of `connect` discards `const` from
 *    pointer target type
 *
 *        Version:  1.0
 *        Created:  07/05/13 23:53:49
 *       Revision:  none
 *       Compiler:  gcc
 *
