...
#include "../config.h" /*configuration options for current OS, "../config.h" is generated by configure*/
...
/* $$.Ic INET6_ADDRSTRLEN$$ */
#define INET6_ADDRSTRLEN 46 /*max size of IPv6 address string:"xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx" or "xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:ddd.ddd.ddd.ddd\0" 1234567890123456789012345678901234567890123456*/
#endif
/* *INDENT-ON* */
 ...
 * an ALIGN() macor, but each implementation does this differently.*/
#ifndef CMSG_LEN
/* $$.Im CMSG_LEN$$ */
 ...
 * all the STREAMS stuff if it's not needed, so we just Define INFITM here.
 * This is the standard value, but there's no quarantee it is -1.
 */
#ifndef INFTIM
...
/* Following could be derived from SOMAXCONN in <sys/socket.h>, but
 * many kernels still #define it as 5, while actually supporting
 * many more
 */
...
uint8_t *sctp_pdapi_recvmsg(int sock_fd, int *rdlen, SA *from, int *from_len, struct sctp_sndrcvinfo *sri, int *msg_flags);
...
int sctp_heartbeat_action(int sock_fd, struct sockaddr *sa, int action, u_int value);
...
/*prototypes for our Unix wrapper functions: set {Sec errors}*/
...
/*prototypes for our stdio wrapper functions: set {Sec errors}*/
void Fclose(FILE *);
...
/*prototypes for our socket wrapper functions: set {Sec errors}*/
...
void *Inet6_rth_int(void *, socklen_t, int, int);
