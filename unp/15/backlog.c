/*************************************************************************
	> File Name: backlog.c
	> Description: exercise 15.4
	> Author: Taylor
	> Mail: 
	> Created Time: 2013年12月17日 星期二 18时34分59秒
 ************************************************************************/
#include "unp.h"
...
void do_child(void){
	...
	while(backlog >= 0){
		...
		Write(cfd, &junk, sizeof(int));/*tell parent,junk is the unknown int.*/

		Read(cfd, &backlog, sizeof(int));/*just wait for parent, then start another listen.*/
